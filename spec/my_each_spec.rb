...
describe "my_each" do
  ...
  it "calls on while" do
    expect(file).to_not include("while")
  end
...
#  end
  ...
  it "can handle an empty collection" do
    ...
    expect(saved_block).to raise_error
  end
...
#  end).to_not contain_exactly('arel', 'jon', 'logan', 'spencer')
#  end
...
#    expect(tas).to_not eq(tas_original)
#  end
...
#    expect(times_called).to_not eq(expected)
#  end
...
#      expect(ta.kind_of?(Array)).to_not eq(false)
